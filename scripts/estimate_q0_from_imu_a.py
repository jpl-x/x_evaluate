...
from x_evaluate.utils import get_ros_topic_name_from_msg_type
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Reads first IMU measurements from bag and calculates the attitude q0 "
                                                 "that would bring us closest to a z-up gravity-down "
                                                 "reference frame. This is done by assuming zero body acceleration in "
                                                 "the first IMU message, i.e. assuming that the gravity vector can be"
                                                 "observed. If pose groundtruth is available in the bag, results are "
                                                 "compared to it")
    ...
    if gt_topic is not None:
        ...
        print(F"z-direction in body frame [EST]: {list(e_z_body_hat)}")
