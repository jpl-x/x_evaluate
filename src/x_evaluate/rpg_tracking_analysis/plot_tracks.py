...
def check_config(config, root, config_path):
    ...
    for element in config:
        ...
        assert len(
            value) == 2, "Individual methods in '%s' should contain only color and folder location." % config_path
...
def plot_tracks_3d(frame_dataset, est_file, gt_file=None, t_max=-1, method="estimation"):
    ...
    ax.plot_surface(t_img - t_min, X, Y, cstride=1, rstride=1, facecolors=img, linewidth=1, shade=False,
                    edgecolor='none')
...
def plot_num_features(f, config_path=None, root=None, error_threshold=10, method="estimation", color=[0, 150, 0]):
    ...
    # parse config
    labels = [list(v.keys())[0] for v in config]
    files = [list(v.values())[0][1:] for v in config]
    colors = [list(v.values())[0][0] for v in config]
    ...
    for label, folders, color in zip(labels, files, colors):
        ...
        # load and parse errors
        errors_dict, tracks_dict = load_method(os.path.join(root, folders[0]))
    ...
    ax.set_xticklabels(labels=x_ticks, fontdict=fontProperties)
    ax.yaxis.set_major_locator(mticker.FixedLocator(ax.get_yticks()))
    ax.set_yticklabels(labels=y_ticks, fontdict=fontProperties)
