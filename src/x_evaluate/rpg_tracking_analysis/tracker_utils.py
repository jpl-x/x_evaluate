...
def project_landmarks(landmarks_global, pose, K):
...
def compare_tracks(est_track_data, gt_track_data):
    ...
    for track_id, est_track in tqdm.tqdm(est_track_data.items()):
        ...
        gt_t, e_x, e_y = get_error(est_track, gt_track)
...
def q_to_R(q):
    ...
    return np.dstack([w ** 2 + x ** 2 - y ** 2 - z ** 2, 2 * x * y - 2 * w * z, 2 * x * z + 2 * w * y, \
                      ...
                      2 * x * z - 2 * w * y, 2 * y * z + 2 * w * x, w ** 2 - x ** 2 - y ** 2 + z ** 2]).reshape(len(w),
                                                                                                                3, 3)
