...
from x_evaluate.rpg_tracking_analysis.tracker import Tracker
from x_evaluate.rpg_tracking_analysis.tracker_init import TrackerInitializer
from x_evaluate.rpg_tracking_analysis.tracker_utils import get_track_data, compare_tracks
from x_evaluate.rpg_tracking_analysis.dataset import Dataset
from x_evaluate.rpg_tracking_analysis.plot_tracks import plot_tracks_3d, plot_num_features, format_summary
from x_evaluate.rpg_tracking_analysis.visualizer import FeatureTracksVisualizer


def rpg_evaluate_tracks(args, dataset_config=None, tracker_config=None):
    ...
    # make feature init
    tracks_init = TrackerInitializer(args.root, dataset_config, config=tracker_init_config)
    ...
    est_tracks_data = get_track_data(args.file, filter_too_short=True)
    gt_tracks_data = get_track_data(out_csv)

    # compute errors
    error_data = compare_tracks(est_tracks_data, gt_tracks_data)
    ...
    if args.plot_3d:
        ...
        print(
            "Saving 3d space-time curves to results/3d_plot.pdf and results/3d_plot_with_gt.pdf. This may take some time.")
        dataset = Dataset(args.root, dataset_config, dataset_type="frames")
        fig1, ax1 = plot_tracks_3d(dataset, args.file, out_csv, t_max=2)
        # fig1.savefig(join(results_folder, "3d_plot_with_gt.pdf"), bbox_inches="tight")  # BUG: matplotlib API changed?
        fig1.savefig(join(results_folder, "3d_plot_with_gt.pdf"))

        dataset = Dataset(args.root, dataset_config, dataset_type="frames")
        fig2, ax2 = plot_tracks_3d(dataset, args.file, t_max=2)
        # fig2.savefig(os.path.join(results_folder, "3d_plot.pdf"), bbox_inches="tight")  # BUG: matplotlib API changed?
    ...
    if args.video_preview:
        ...
        viz.write_to_video_file(out_video)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--plot_errors', help="Tracks file giving KLT initialization.", action="store_true",
                        default=False)
    parser.add_argument('--error_threshold', help="Error threshold. Tracks which exceed this threshold are discarded.",
                        type=float, default=10)
    parser.add_argument('--video_preview', help="Whether to create a video preview.", action="store_true",
                        default=False)

    arguments = parser.parse_args()
    rpg_evaluate_tracks(arguments)
