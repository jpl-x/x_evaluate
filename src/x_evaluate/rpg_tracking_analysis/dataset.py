...
from x_evaluate.rpg_tracking_analysis.tracker_utils import slerp, q_to_R, interpolate
from x_evaluate.rpg_tracking_analysis.bag2dataframe import Bag2Images, Bag2DepthMap, Bag2Trajectory, Bag2CameraInfo


class Dataset:
    def __init__(self, root, config, dataset_type="images"):
        self.root = root
        ...
        self.queue, self.times, self.config = load(root, config)

        self.it = 0

    def _load(self, root, config, topic, cls, *attrs):
        """
        load
        """
        ...
        if config["type"] == "bag":
            assert topic in config, "The dataset config at needs a key with name '%s'." % topic
